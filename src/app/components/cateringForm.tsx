...
// ── Data ───────────────────────────────────────────────────────────────────────
// ✅ These IDs must match the `slug` field in Catering.tsx PACKAGES
const PACKAGES = [
  {
    id: "super-deluxe",
...
// ── Read ?package= from URL — no router needed ────────────────────────────────
function getPackageFromURL(): string {
  const params = new URLSearchParams(window.location.search);
  const slug = params.get("package") ?? "";
  const valid = PACKAGES.some((p) => p.id === slug);
  return valid ? slug : "";
}
...
export default function CateringOrderForm() {
  const [form, setForm] = useState<FormState>({
    ...
    eventTime: "",
    guestCount: "",
    selectedPackage: getPackageFromURL(), // ✅ pre-selects from URL on load
    extras: [],
    notes: "",
  });
    ...
    return (
      <div className="min-h-screen bg-[#FAF6EE] flex items-center justify-center px-6">
        <div className="text-center max-w-md animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <h2 className="font-serif text-3xl font-bold text-stone-800 mb-3">Order Received!</h2>
          <p className="text-stone-500 leading-relaxed mb-2">
            Thank you,{" "}
            <strong className="font-semibold text-stone-700">{form.fullName}</strong>! Your
            catering request has been submitted successfully.
          </p>
          <p className="text-stone-500 leading-relaxed mb-8">
            We'll reach out to{" "}
            <span className="text-stone-700 font-medium">{form.email}</span> or{" "}
            <span className="text-stone-700 font-medium">{form.phone}</span> to confirm shortly.
          </p>
          <Button
            onClick={() => {
              setStatus("idle");
  ...
  return (
    <div className="min-h-screen bg-[#FAF6EE] font-sans">
      ...
      <nav className="fixed top-0 w-full z-50 bg-stone-900/97 backdrop-blur-sm border-b border-amber-600/20 flex items-center justify-between px-[5%] h-16">
        ...
        >
          <span className="text-white/40 text-sm not-italic">←</span>
          "<span className="text-amber-500">Your</span>" Pizza Shop
          <span className="text-white/40 text-sm font-normal not-italic ml-1">· Largo</span>
        </a>
        <a
          href="tel:7275811101"
      ...
      <div className="pt-16 grid lg:grid-cols-[1fr_380px] min-h-screen">
          ...
          <form onSubmit={handleSubmit} className="space-y-10 max-w-2xl">
            ...
            <fieldset className="space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <Select required value={form.eventType} onValueChange={(v) => setForm((p) => ({ ...p, eventType: v }))}>
              ...
              <div className="grid sm:grid-cols-3 gap-5">
                <div className="space-y-1.5">
                  ...
                  <Input id="eventDate" type="date" value={form.eventDate} onChange={set("eventDate")} required min={new Date().toISOString().split("T")[0]} className="rounded-none border-stone-200 focus-visible:ring-red-500" />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="eventTime" className="text-xs font-semibold tracking-wider uppercase text-stone-500">Start Time</Label>
                  <Input id="eventTime" type="time" value={form.eventTime} onChange={set("eventTime")} className="rounded-none border-stone-200 focus-visible:ring-red-500" />
                </div>
                <div className="space-y-1.5">
                  ...
                  <Input id="guestCount" type="number" min="1" value={form.guestCount} onChange={set("guestCount")} placeholder="e.g. 30" required className="rounded-none border-stone-200 focus-visible:ring-red-500" />
                </div>
              </div>
            </fieldset>
            ...
            <fieldset className="space-y-5">
              ...
              <div className="space-y-3">
                {PACKAGES.map((pkg) => {
                  ...
                  return (
                    <label key={pkg.id} htmlFor={pkg.id}
                      className={`flex items-start gap-4 p-4 border cursor-pointer transition-all duration-200 ${isSelected ? "border-red-500 bg-red-50" : "border-stone-200 bg-white hover:border-stone-300"}`}
                    >
                      <input type="radio" id={pkg.id} name="selectedPackage" value={pkg.id} required checked={isSelected}
                        onChange={() => setForm((p) => ({ ...p, selectedPackage: pkg.id }))}
            ...
            <fieldset className="space-y-5">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {EXTRAS.map((extra) => {
                  ...
                  return (
                    <label key={extra.id} htmlFor={extra.id}
                      className={`flex items-start gap-3 p-3.5 border cursor-pointer transition-all duration-150 ${checked ? "border-amber-400 bg-amber-50" : "border-stone-200 bg-white hover:border-stone-300"}`}
                    >
                      <Checkbox id={extra.id} checked={checked} onCheckedChange={() => toggleExtra(extra.id)}
            ...
            <p className="text-center text-xs text-stone-400">
              ...
              <a href="tel:7275811101" className="text-red-600 font-semibold hover:underline">(727) 581-1101</a>
            </p>
          </form>
        </div>
        ...
        <aside className="hidden lg:flex flex-col gap-6 bg-stone-900 px-8 py-16 sticky top-16 self-start h-[calc(100vh-64px)] overflow-y-auto">
          <div>
            <p className="text-amber-400 text-[10px] font-bold tracking-[0.2em] uppercase mb-4">Order Summary</p>
                ...
                {form.selectedPackage ? (
                  <>
                    <div>
                      ...
                      <p className="text-white text-sm font-semibold leading-snug">{selectedPackageData?.label}</p>
                      ...
                      <div>
                        ...
                        <div className="space-y-1">
                          {form.extras.map((id) => {
                            ...
                            return <p key={id} className="text-white/70 text-xs leading-relaxed">+ {extra?.label}</p>;
                          })}
                        </div>
                      </div>
                    )}
                  ...
                  <p className="text-white/30 text-sm italic">Your selections will appear here…</p>
                )}
              </CardContent>
            </Card>
          </div>
          ...
          {(form.eventDate || form.eventType) && (
            <div className="space-y-3">
              <p className="text-amber-400 text-[10px] font-bold tracking-[0.2em] uppercase">Event Info</p>
              {form.eventType && <p className="text-white/60 text-sm">{form.eventType}</p>}
              {form.eventDate && (
                <p className="text-white/60 text-sm">
                  {new Date(form.eventDate + "T00:00").toLocaleDateString("en-US", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
          ...
          <div className="space-y-3">
            <p className="text-amber-400 text-[10px] font-bold tracking-[0.2em] uppercase mb-3">Contact Us</p>
            <div className="flex items-start gap-3">
              <Phone className="w-4 h-4 text-white/30 mt-0.5 flex-shrink-0" />
              <a href="tel:7275811101" className="text-white/60 text-sm hover:text-white transition-colors no-underline">(727) 581-1101</a>
            </div>
            <div className="flex items-start gap-3">
